...
\title{Learning complex stochastic models with invertible neural networks: a likelihood-free approach to Bayesian inference}
...
\caption[short]{Results on the \textit{Splat} scRNA simulation model. \textbf{(a)} The \textit{Splat} simulation diagram; \textbf{(b)} Parameter recovery; \textbf{(c)} Simulation-based calibraton. The SCB plots indicate a slight underestimation of the true posterior variance.  \textbf{(d)} Example full parameter posteriors from three datasets.} \label{fig:Fig.7}
\end{figure}
The results on the scRNA example are depicted in \autoref{fig:Fig.7}. First, recovery of most parameters is very good, with maximum $R^{2}$ of $0.98$ and minimum NRMSE of $0.039$ for the library location parameter $\mu^{L}$. Interestingly, the rate parameter $\beta$ appears to be nearly unrecoverable, as indexed by the scatterplots in \autoref{fig:Fig.7b}. Furthermore, the estimated posterior of $\beta$ is often bimodal, covering the whole prior range (\autoref{fig:Fig.7c}). This might indicate that the parameter is not identifiable, or that some multivariate trade-offs exist in the parameter space of the \textit{Splat} model. The latter is also confirmed by observing a few marked outliers in the scatterplots of $\alpha$, $\sigma^{O}$, and $\pi^{O}$. Finally, the SCB histograms indicate that our method tends to slightly underestimate the true posterior variance of the $\alpha$, $\beta$, $\mu^{O}$ $\pi^{O}$, and $\sigma^{O}$ parameters. This issue might be due to the fact that the networks were trained with the reference-table approach and might disappear if the online learning approach is employed.
