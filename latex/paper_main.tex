...
\begin{figure}
    \centering
    \subfloat[Training phase]{{\includegraphics[width=.99\linewidth]{training.png}}}
    \newline
    \subfloat[Inference phase]{{\includegraphics[width=.99\linewidth]{inference.png}}}
...
EAMs are particularly amenable for likelihood-free inference, since most members of the family turn out to be intractable. This intractability has precluded many interesting applications and empirically driven model refinements. Here, we apply our method to estimate the parameters of the recently proposed Lévy-Flight Model (LFM, cite). The LFM assumes an \textit{alpha-stable} noise distribution of the evidence accumulation process in order to model "jumps" in the decision process. The LFM does not have a known likelihood and a fully Bayesian treatment of the model is still missing from the literature. The forward equation of the LFM is given by:
\begin{align*}
