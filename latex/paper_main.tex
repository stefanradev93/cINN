...
Second, we showed that the ML loss directly maximizes the posterior over the target model parameters without any assumptions on the shape of the posterior. This is in contrast to ELBO-based methods which optimize a lower-bound on the posterior \cite{papamakarios2016fast, kingma2014auto}, and usually assume Gaussian posteriors. Therefore, posterior inference is exact when the ML is globally minimized \cite{kingma2018glow, dinh2016density}. Our results support this theoretical claim. Further, since researchers are often interested in some summary of the posterior (e.g., posterior means or variances), we also showed that our method exhibits excellent recovery of the posterior means throughout all examples. Further, parameter recovery becomes better with increasing number of observations, whereas the posterior variance of the parameters decreases. These are important and highly desirable properties of any Bayesian parameter estimation method, as it mirrors the increase in information following increasing number of observations. 
