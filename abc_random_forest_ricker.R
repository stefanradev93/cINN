...
rf = regAbcrf(r ~ ., data=train_data, ntree=200)
...
rf = regAbcrf(sigma ~ ., data=train_data, ntree=200)
...
rf = regAbcrf(rho ~ ., data=train_data, ntree=200)
...
rf = regAbcrf(u ~ ., data=train_data, ntree=200)
...
write.table(theta_hat, "./sim_data/ricker_results/theta_means_rf.csv", sep=';')
